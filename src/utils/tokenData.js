...
Promise.all([
  ...
  const contract = new Contract(PAIR_ADDRESS.LUA_USDC, luaAbi, provider);
  contract.price0CumulativeLast().then((result) => {
    console.log("Price 0:", result, new BigNumber(result._hex, 18).toFixed(2));
  });
});
